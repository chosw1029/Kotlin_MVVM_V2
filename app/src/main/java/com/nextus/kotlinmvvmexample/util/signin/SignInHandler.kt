...
package com.nextus.kotlinmvvmexample.util.signin
...
interface SignInHandler {
    ...
    fun makeGoogleSignInIntent(): LiveData<Intent?>
    fun makeEmailSignInIntent(): LiveData<Intent?>
...
class FirebaseAuthSignInHandler(
     ...
     */
    override fun makeGoogleSignInIntent(): LiveData<Intent?> {
    ...
    override fun makeEmailSignInIntent(): LiveData<Intent?> {
        val result = MutableLiveData<Intent?>()
        ...
        //FirebaseAuth.getInstance().createUserWithEmailAndPassword(email.value!!, password.value!!)
                /*.addOnCompleteListener(signUpActivity!!) { task ->
                    if (task.isSuccessful) {
                        ...
                        AppLogger.e("Success!! : ${user.toString()}")

                        // 서버에 유저 등록
                        createUserInfoAtServer(user!!.uid)
                    } else {
                        // If sign in fails, display a message to the user.
                        AppLogger.e("createUserWithEmail:failure ${task.exception}")
                        showSnackBar("${task.exception}")
                    }
